...
actor MarketplaceCanister {
  ...
  public shared ({ caller }) func listLand(tokenId : Nat, price : Nat) : async Nat {
    ...
    // SECURITY: Verify land canister is configured before allowing listings
    let landCanister = switch (landCanisterPrincipal) {
      case null {
        Debug.trap("Configuration error: Land canister must be configured by admin before marketplace operations are enabled");
      };
      case (?canisterId) {
  ...
  public shared ({ caller }) func buyLand(listingId : Nat) : async BuyResult {
    ...
    switch (natMap.get(listings, listingId)) {
      ...
      case (?listing) {
        ...
        // SECURITY: Verify canisters are configured before allowing purchases
        let landCanister = switch (landCanisterPrincipal) {
          case null {
            Debug.trap("Configuration error: Land canister must be configured by admin before marketplace operations are enabled");
          };
        ...
        let tokenCanister = switch (tokenCanisterPrincipal) {
          case null {
            Debug.trap("Configuration error: Token canister must be configured by admin before marketplace operations are enabled");
          };
          case (?canisterId) {
            actor (Principal.toText(canisterId)) : actor {
  ...
  public shared ({ caller }) func cancelListing(listingId : Nat) : async () {
    ...
    switch (natMap.get(listings, listingId)) {
      ...
      case (?listing) {
        ...
        // SECURITY: Verify land canister is configured before allowing cancellations
        let landCanister = switch (landCanisterPrincipal) {
          case null {
            Debug.trap("Configuration error: Land canister must be configured by admin before marketplace operations are enabled");
          };
  ...
  public shared ({ caller }) func adminCancelListing(listingId : Nat) : async () {
    ...
    switch (natMap.get(listings, listingId)) {
      ...
      case (?listing) {
        // SECURITY: Verify land canister is configured before allowing admin cancellations
        let landCanister = switch (landCanisterPrincipal) {
          case null {
            Debug.trap("Configuration error: Land canister must be configured by admin before marketplace operations are enabled");
          };
