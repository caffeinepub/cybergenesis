...
actor AssetCanister {
  ...
  // SECURITY FIX: Only authorized admin principal can initialize access control
  // This prevents unauthorized users from becoming admin
  public shared ({ caller }) func initializeAccessControl() : async () {
    // STRICT GUARD: Only the authorized admin principal can initialize access control
    if (caller != authorizedAdminPrincipal) {
      Debug.print("Unauthorized initializeAccessControl attempt by: " # debug_show(caller) # " Expected: " # debug_show(authorizedAdminPrincipal));
      Debug.trap("Unauthorized: Only the authorized admin principal (whd5e-pbxhk-pp65k-hxqqx-edtrx-5b7xd-itunf-pz5f5-bzjut-dxkhy-4ae) can initialize access control");
    };
    
    Debug.print("Asset Canister access control initialization by authorized admin: " # debug_show(caller));
  ...
  // Asset Management Functions - SECURITY FIX: Verify caller is actually the GovernanceCanister

  public shared ({ caller }) func uploadAsset(filename : Text, data : Blob) : async () {
    // SECURITY FIX: Verify caller is the configured GovernanceCanister
    switch (governanceCanister) {
      ...
      case (?governance) {
        if (caller != governance) {
          Debug.trap("Unauthorized: Only the configured GovernanceCanister can upload assets");
        };
      };
    };
  ...
  public shared ({ caller }) func batchUploadAssets(assetList : [(Text, Blob)]) : async () {
    ...
    switch (governanceCanister) {
      ...
      case (?governance) {
        if (caller != governance) {
          Debug.trap("Unauthorized: Only the configured GovernanceCanister can batch upload assets");
        };
      };
    };
  ...
  public shared ({ caller }) func deleteAsset(filename : Text) : async () {
    ...
    switch (governanceCanister) {
      ...
      case (?governance) {
        if (caller != governance) {
          Debug.trap("Unauthorized: Only the configured GovernanceCanister can delete assets");
        };
      };
    };
